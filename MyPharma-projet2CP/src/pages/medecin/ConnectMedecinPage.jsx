...
import { Link } from 'react-router-dom';
  ...
  const handleLogin = (e) => {
    ...
    // Simuler la connexion en récupérant l'inscription Etape1
    let profilStr = localStorage.getItem("medecinEtape1");
    let profil = profilStr ? JSON.parse(profilStr) : null;
    
    // Si l'email ne correspond pas au profil inscrit, on simule un Docteur par défaut
    if (!profil || profil.email !== email) {
        profil = { 
            nom: "Dr. Connecté", 
            email: email, 
            telephone: "0000000000",
            specialite: "Médecin Connecté",
            adresse: "Adresse de Test"
        };
    }
    
    // Actualiser la session
    localStorage.setItem("medecinConnecte", JSON.stringify(profil));
    window.location.href = "/profil"; // Rafraîchissement brutal pour recharger le localStorage sur toutes les pages
  };
