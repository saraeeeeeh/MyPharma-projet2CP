...
const PlanningPage = () => {
  ...
  useEffect(() => {
    if (data && data.id) {
      fetch(`/api/appointments?medecinId=${data.id}`)
        .then(res => res.json())
        .then(fetchedEvents => {
          const formatted = fetchedEvents.map(e => ({
              ...
              date: e.date,
              time: e.heure,
              duration: 30, // on met 30 min par défaut s'il n'y a pas de durée dans la table
              patient: e.patientNom || `${e.patientPrenom || ''} ${e.patientNom || ''}`.trim() || 'Patient inconnu',
              type: e.motif,
              color: "#137FEC" // Couleur par défaut
          }));
          setEvents(formatted);
        })
  ...
  const addEvent = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Erreur sauvegarde rdv");
        ...
        <div className="modal-overlay" onClick={() => setSelectedEventInfo(null)}>
            <div className="modal-content form-card" style={{maxWidth: "400px"}} onClick={e => e.stopPropagation()}>
                ...
                <div className="ligne-horizontale" style={{marginTop:"30px"}}>
                     <button className="FIN" style={{backgroundColor: "#dc2626", color: "white"}} onClick={() => { 
