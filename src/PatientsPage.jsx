...
const PatientsPage = () => {
  ...
  useEffect(() => {
    if (data && data.id) {
      fetch(`/api/patients?medecinId=${data.id}`)
        .then(res => res.json())
        .then(fetchedPatients => {
          // On map les données pour correspondre au format d'affichage
          const formatted = fetchedPatients.map(p => ({
            ...p,
            age: p.dateNaissance || 0, // Dans l'ajout on met l'âge dans dateNaissance
            rdv: "Aucun",
  ...
  const handleAddPatient = async (e) => {
    ...
    try {
      const response = await fetch('/api/patients', {
        ...
        body: JSON.stringify({
          nom: newPatient.nom,
          prenom: "", // On pourrait séparer nom et prenom dans le form
          dateNaissance: newPatient.age,
